...
impl<'a, T:Sync> EvilRcu<T>{
    ...
    fn write(&'a self) -> RcuWriteGuard<'a, T>{
        RcuWriteGuard{data: self.vestiges.lock().unwrap(), lock: &self} 
    }

    fn try_write(&'a self) -> Option<RcuWriteGuard<'a, T>>{
        self.vestiges.try_lock().ok().map(|data|RcuWriteGuard{ data, lock: &self} )
    }
...
impl<T:Sync> EvilRcuInner<T>{
    ...
    fn replace(&self, new:Arc<T>) -> Arc<T>{
        std::mem::replace(unsafe{self.0.get().as_mut()}.unwrap(), new)
    }
}
